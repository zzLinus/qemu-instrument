...
{
    TRANSLATION_DATA *t = &tr_data;
    lsassertm(t->cur_INS_nr < MAX_PIN_INS_NR, "too many INS\n");
    INS INS = &(t->INS_array[t->cur_INS_nr]);
    t->cur_INS_nr++;
...
{
    TRANSLATION_DATA *t = &tr_data;
    lsassertm(t->cur_BBL_nr < MAX_PIN_BBL_NR, "too many BBL\n");
    BBL bbl = &(t->BBL_array[t->cur_BBL_nr]);
    t->cur_BBL_nr++;
...
{
    TRANSLATION_DATA *t = &tr_data;
    lsassertm(t->cur_TRACE_nr < MAX_PIN_TRACE_NR, "too many TRACE\n");
    TRACE trace = &(t->TRACE_array[t->cur_TRACE_nr]);
    t->cur_TRACE_nr++;
